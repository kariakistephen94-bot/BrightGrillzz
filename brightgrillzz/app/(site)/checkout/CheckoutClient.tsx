...
export default function CheckoutClient() {
  ...
  // Live from admin Settings: bank transfer details + whether Paystack is offered.
  const settings = useSiteSettings()
  const acceptOnlinePayments = settings.acceptOnlinePayments
  const paymentRows = [
  ...
  // The methods the customer may pick. Paystack only appears when the admin has
  // online payments enabled.
  const paymentOptions: { value: PaymentMethod; title: string; sub: string }[] = [
    { value: 'bank_transfer', title: 'Bank transfer', sub: 'Transfer to our account, then confirm' },
    ...(acceptOnlinePayments
      ? [{ value: 'paystack' as const, title: 'Pay online', sub: 'Card, transfer or USSD via Paystack' }]
      : []),
  ]

  // Safety net: even if state somehow held 'paystack', a disabled toggle forces
  // bank transfer so the online flow can never run.
  const effectiveMethod: PaymentMethod = acceptOnlinePayments ? paymentMethod : 'bank_transfer'
  ...
  const validate = () => {
    ...
    if (effectiveMethod === 'bank_transfer' && !paymentConfirmed) {
          ...
          <div className="lg:col-span-2">
            <div className="glass-card rounded-2xl md:rounded-[2rem] p-4 sm:p-5 md:p-6 lg:p-8 sticky top-28 lg:top-32">
                  ...
                  <div key={item.cartId} className="flex gap-2 sm:gap-3 items-center">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] sm:text-xs text-muted-foreground">
                        {item.qty} × {formatNaira(item.price ?? 0)}
                      </p>
                    </div>
                    <p className="font-bold text-xs sm:text-sm shrink-0">{formatNaira((item.price ?? 0) * item.qty)}</p>
                  </div>
                ))}
              </div>
              ...
              <Button
                ...
                disabled={phase !== 'idle'}
              >
                {effectiveMethod === 'paystack' ? `Pay ${formatNaira(total)} securely` : 'Place order'}
